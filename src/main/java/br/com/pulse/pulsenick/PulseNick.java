...
public final class PulseNick extends JavaPlugin {
     ...
     * Responsible for loading support, connecting to the database, starting configs, and registering commands and events.
     */
    @Override
    public void onEnable() {
        ...
        try {
            ...
            utility.warn("&cAn error occurred during plugin startup: &f" + e.getMessage());
            Bukkit.getPluginManager().disablePlugin(this);
            return;
        }

        long endTime = System.currentTimeMillis();
        utility.info("&aPlugin enabled in &f" + (endTime - startTime) + "ms");
    }
    ...
    public void onDisable() {
        ...
        if (connection != null) {
            try {
                connection.close();
                utility.info("&aDatabase connection closed successfully.");
            } catch (SQLException e) {
                utility.warn("&cError closing database connection: &f" + e.getMessage());
            }
        }

        long endTime = System.currentTimeMillis();
        utility.info("&cPlugin disabled in &f" + (endTime - startTime) + "ms");
    }
    ...
    private void connectDatabase() {
        ...
        try {
            ...
            long endTime = System.currentTimeMillis();
            utility.info("&aDatabase connection established in &f" + (endTime - startTime) + "ms");

        } catch (SQLException e) {
            utility.warn("&cCould not connect to database: &f" + e.getMessage());
            Bukkit.getPluginManager().disablePlugin(this);
        }
    }
    ...
    private void registerListeners() {
        ...
        long endTime = System.currentTimeMillis();
        utility.info("&aListeners registered in &f" + (endTime - startTime) + "ms");
    }
    ...
    private void registerCommands() {
        ...
        long endTime = System.currentTimeMillis();
        utility.info("&aCommands registered in &f" + (endTime - startTime) + "ms");
    }
    ...
    private void registerPlaceholders() {
        ...
        if (papi != null) {
            ...
            utility.info("&aPlaceholderAPI expansion registered.");
        } else {
            utility.warn("&cPlaceholderAPI not found. Expansion not registered.");
        }
    }
}